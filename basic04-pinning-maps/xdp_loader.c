...
int pin_maps_in_bpf_object(struct bpf_object *bpf_obj, const char *subdir)
{
	...
	char map_basefilename[PATH_MAX];
	...
	len = snprintf(map_basefilename, PATH_MAX, "%s/%s",
	...
	printf("map_basefilename: %s\n", map_basefilename);
	if (access(map_basefilename, F_OK ) != -1 ) {
		if (verbose)
			printf(" - Unpinning (remove) prev maps in %s/\n",
			       pin_basedir);

		err = bpf_object__unpin_maps(bpf_obj, pin_basedir);
		if (err) {
			fprintf(stderr, "ERR: UNpinning maps in %s\n", pin_basedir);
			return EXIT_FAIL_BPF;
		}
	...
	err = bpf_object__pin_maps(bpf_obj, pin_basedir);
	if (err)
		return EXIT_FAIL_BPF;
